...
public class TerrainView extends View {
  ...
  protected void onDraw(@NonNull Canvas canvas) {
    ...
    // DONE Do the following ONLY IF neither terrain nor breedPaints is null. THIS IS IMPORTANT!
    if (terrain != null && breedPaints != null) {
      ...
      float cellSize = Math.min(
          (float) getWidth() / terrain.length,
          (float) getHeight() / terrain.length
      );
      ...
      // Done Iterate over all the rows and columns of terrain, using a traditional for loop
